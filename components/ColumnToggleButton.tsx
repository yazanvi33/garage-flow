...
const ColumnToggleButton: React.FC<ColumnToggleButtonProps> = ({ allColumns, visibleColumns, onToggleColumn }) => {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button as={Button} variant="outline" size="md" rightIcon={ChevronDownIcon} leftIcon={ViewColumnsIcon}>
          {getLabel('columns')}
        </Menu.Button>
      </div>
      <Transition
              ...
              <Menu.Item key={col.key}>
                {({ active }) => (
                  <label
                    ...
                    } group flex rounded-md items-center w-full px-3 py-2 text-sm text-gray-700 dark:text-gray-200 cursor-pointer`}
                  >
                    <input
                      type="checkbox"
